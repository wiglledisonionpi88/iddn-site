...
class AZRTradingBot:
    ...
    def analyze_price_data(self, symbol: str) -> Dict:
        ...
        if self.demo_mode:
            ...
            return {
                ...
                'avg_change': recent_change * 0.8,
                'volatility': abs(recent_change) * 2,
                'volume_ratio': random.uniform(0.8, 2.0),
                'analysis': f"DEMO: Price change: {recent_change:.2f}%, Volume: {random.uniform(0.8, 2.0):.2f}x"
            }
    ...
    def execute_trade(self, symbol: str, signal: str, position_size: float) -> Optional[Dict]:
        ...
        if self.demo_mode:
            ...
            trade_log = {
                ...
                'amount': position_size / random.uniform(45000, 55000) if 'BTC' in symbol else position_size / random.uniform(2500, 3500),
                'cost': position_size,
                'fee': {'cost': position_size * 0.001, 'currency': 'USDT'},
                'price': random.uniform(45000, 55000) if 'BTC' in symbol else random.uniform(2500, 3500),
                'demo': True
            }
            
            self.logger.info(f"DEMO TRADE: {signal} {symbol} - Amount: {trade_log['amount']:.6f}, Cost: ${trade_log['cost']:.2f}")
            return trade_log
    ...
    def run_trading_cycle(self):
        ...
        self.logger.info(f"Starting trading cycle at {cycle_start} - {mode_text}")
        
        # Live trading warning
        if not self.sandbox_mode and not self.demo_mode:
            self.logger.warning("🔴 LIVE TRADING MODE - REAL MONEY AT RISK")
        elif self.demo_mode:
            self.logger.info("🟡 DEMO MODE - No real trading, simulating for testing")
